...
*/
if(file_exists("config_conexao.php")) {			
	include "config_conexao.php";

	if (!defined("SERVIDOR") or !defined("USUARIO") or !defined("SENHA") or !defined("BANCO")){
		echo "Variaveis de conexao nao definidas, configure corretamente o arquivo conecta.php";
		exit;
	}
}
...
function Abre_Conexao() {	
	global $erros;
	@mysql_connect("localhost", "myke", "");
	if(mysql_errno() != 0) {
		echo $erros[mysql_errno()];	
		exit;	
	}	
	@mysql_select_db("estilos");		
	if(mysql_errno() != 0) {
		echo $erros[mysql_errno()];	
		exit;
	}		
